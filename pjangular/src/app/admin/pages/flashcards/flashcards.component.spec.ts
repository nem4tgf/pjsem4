...
export class FlashcardsComponent implements OnInit {
  ...
  onSortChange(sortBy: string, sortDir: string | null): void {
    console.log('[FlashcardsComponent] Sort changed: sortBy=', sortBy, 'sortDir=', sortDir);
    // Chuyển đổi sortDir thành kiểu hợp lệ
    const validSortDir: 'ascend' | 'descend' | null = sortDir === 'ascend' || sortDir === 'descend' ? sortDir : null;
    let backendSortDir: 'ASC' | 'DESC' | undefined;

    if (validSortDir === 'ascend') {
      backendSortDir = 'ASC';
    } else if (validSortDir === 'descend') {
      backendSortDir = 'DESC';
    } else {
      backendSortDir = 'ASC'; // Mặc định là 'ASC' khi sortDir là null
    }
